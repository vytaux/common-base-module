...
public class SyncJobs {
	...
	private Map<ScheduleType, List<Runnable>> syncServicesMap = new HashMap<>();
	...
	private void runSchedule(ScheduleType scheduleType) {
		...
		List<Runnable> services = syncServicesMap.get(scheduleType);
	...
	public synchronized boolean addSyncService(Runnable service, ScheduleType scheduleType) {
		List<Runnable> serviceList = syncServicesMap.get(scheduleType);
		if(null == serviceList) {
			serviceList = new ArrayList<>();
			syncServicesMap.put(scheduleType, serviceList);
		}
		else {
